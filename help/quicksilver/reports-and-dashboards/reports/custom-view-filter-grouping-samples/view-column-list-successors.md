...
# Weergave: Voeg een lijst van taakopvolgers in een kolom toe

U kunt een kolom aan een taakmening toevoegen om een lijst van de opvolgers van de taken te tonen. De **Taakopvolgers** bevat het nummer van de opvolger en de naam.
...
1. Verwijder alle tekst in het vak Tekstmodus en vervang deze door de volgende code:
   ...
   <pre>displayname=Task Successors<br>listdelimiter=<br><br>listmethod=nested(opvolgers).lists<br>textmode=true<br>type=iterate<br>valueexpression=CONCAT({opvolger}.{taskNumber},' - ',{opvolger}.{name})<br>valueformat=HTML</pre>
