...
U kunt dezelfde berekeningen in zowel berekende velden als een berekende kolom gebruiken. Afhankelijk van uw doel voor deze berekeningen, kunt u echter overwegen om het ene versus het andere te bouwen.

## Syntaxis van berekende aangepaste velden versus berekende aangepaste kolommen

Hoewel de functies die u gebruikt hetzelfde zijn, kan de syntaxis voor het bouwen van een expressie in een berekend aangepast veld verschillen van de syntaxis voor het maken van een berekende aangepaste kolom.
...
* In een douanekolom in een rapport, zou u het volgende gebruiken om een de douanekolom van de Naam van het Project op een taakrapport toe te voegen:
  ...
  >[!TIP]
  >
  >Dezelfde syntaxis is van toepassing op alle tekstopmaakelementen waarin berekende expressies worden gebruikt: weergaven, filters, groeperingen en aanwijzingen.

De verschillen tussen de twee syntaxis zijn:
...
<table style="table-layout:auto"> 
 ...
 <tbody> 
  <tr> 
   ...
   <td><strong>Berekend aangepast rapportageelement</strong></td> 
  </tr> 
  <tr> 
...
* Op de belangrijkste pagina van een voorwerp, klikkend het Meer pictogram ![ Meer pictogram ](assets/more-icon.png), dan klikkend **Recalculate Uitdrukkingen**

* Bulk die veelvoudige voorwerpen uitgeven wanneer **de Uitdrukkingen van de Douane** wordt toegelaten opnieuw berekenen.
* Bulk die veelvoudige voorwerpen uitgeven wanneer het klikken **Uitdrukkingen** van het Meer menu in een lijst van voorwerpen opnieuw berekent.
* Het uitgeven van een douaneformulier wanneer **vorige berekeningen van de Update** voor het berekende douanegebied wordt toegelaten.
...
* Wanneer u niet van plan bent om de gegevens samen te voegen voorbij de samenvoeging die in de kolom wordt berekend (de gegevens kunnen slechts eenmaal worden samengevoegd).
* Als u wilt dat de berekening een verwijzing naar de huidige datum bevat met behulp van de jokertekens $$TODAY of $$NOW.

  >[!TIP]
  >
  >Gebruik deze verwijzing niet in berekende aangepaste velden omdat deze alleen opnieuw worden berekend wanneer het gekoppelde object wordt bewerkt. Deze typen berekeningen zijn verouderd.
...
* [Voorbeelden van aangepaste weergaven, filters en groeperingen: artikelindex](../../../reports-and-dashboards/reports/custom-view-filter-grouping-samples/custom-view-filter-grouping-samples.md)
