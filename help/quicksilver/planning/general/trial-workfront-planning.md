---
title: Aan de slag met de gratis proefversie van Adobe Workfront Planning
description: Adobe Workfront biedt een gratis proefperiode van 60 dagen voor Adobe Workfront Planning. Deze proef is uw kans om te zien hoe Planning een grootbeeldoplossingsstrategie verbindt met het dagelijkse werk dat het doet gebeuren. De planning verstrekt gecentraliseerde zichtbaarheid in uw marketing levenscyclus, toelatend betere samenwerking over teams, real time het volgen van campagnes, en geautomatiseerde werkschema's. U kunt aangepaste werkruimten maken en het werk visualiseren met interactieve tijdlijnen en kalenders. In deze proefperiode zult u zien hoe de planning uw dagelijkse werk helpt af te stemmen op strategische doelstellingen, de uitvoering te stroomlijnen en meetbare resultaten voor uw organisatie te stimuleren.
author: Alina
feature: Workfront Planning
role: User, Admin
exl-id: fcad60b2-05e8-4774-8135-129bc1d3f9ce
last-update: 2026-04-01T18:03:50Z
git-commit-file: b03dbe8e217593e0f3a6fcd522148dcd8b7670b8
source-git-commit: 5c4ffeabf710374b14a2335b47342be4c393a7c8
workflow-type: tm+mt
source-wordcount: '2368'
ht-degree: 0%

---


# Ga aan de slag met de gratis proefversie van Adobe Workfront Planning

<!-- are we still calling the tab "Best practice workspaces"? If not, reword below-->

<!--add screen shots-->
<!--check the names of areas, buttons, and links: Learn more, Open Planning, Review trial agreement, etc-->

De Planning van Adobe Workfront verstrekt gecentraliseerde zichtbaarheid in uw marketing levenscyclus, toelatend betere samenwerking over teams, real time het volgen van campagnes, en geautomatiseerde werkschema&#39;s. U kunt aangepaste werkruimten maken en het werk visualiseren met interactieve tijdlijnen en kalenders.

>[!IMPORTANT]
>
>* Adobe Workfront biedt een gratis proefversie van 60 dagen van Adobe Workfront Planning aan alle klanten van Adobe Workfront die niet ook klanten van Workfront Planning zijn.
>
>* De aanmeldperiode eindigde op 1 april 2026 en is nu gesloten.
>
>* Als u de proefovereenkomst hebt geaccepteerd en u bent ingeschreven voor de proefversie, kunt u deze vinden tussen de volgende datums: 2 maart - 1 mei 2026. Toegang blijft actief tot 15 mei 2026.
>
>* De proefversie is 60 dagen beschikbaar vanaf de start en niet 60 dagen vanaf de datum waarop u zich hebt aangemeld.
>

Met deze proefversie zult u eerst zien hoe de planning uw dagelijkse werk helpt af te stemmen op strategische doelstellingen, de uitvoering stroomlijnt en meetbare resultaten voor uw organisatie aanstuurt.

Nadat uw Workfront-beheerder de overeenkomst heeft geaccepteerd en zich heeft aangemeld voor de gratis proefperiode, is Workfront Planning beschikbaar voor iedereen in uw organisatie die op 2 maart 2026 begint.

Het gratis Workfront-planningsonderzoek biedt het volgende:

* Een gekromde, multi-werkruimte planningsomgeving
* Een Workfront Planning Prime-pakket dat de volgende functies bevat:

   * Onbeperkte werkruimten
   * 500.000 records per werkruimte
   * 2 miljoen werkruimten
   * Algemene recordtypen
* Voorbeeldgegevens om u een idee te geven waar u moet beginnen
* Met AI-instructies bij het instappen kunt u gewoon taal gebruiken of een bestaand artefact uploaden. Met Planning kunt u een aangepaste structuur genereren met behulp van AI. Hiermee maakt u automatisch werkruimten, recordtypen, velden en weergaven.
* Training en begeleiding in producten
* Duidelijke aanwijzing van mijlpalen tijdens uw opstelling die voor specifieke rollen worden gemaakt.

## Vereisten voor deelname aan de proefversie

Uw bedrijf moet aan de volgende vereisten voldoen om aan de gratis proefversie van Workfront Planning te kunnen deelnemen:

* Beschikken over een van de volgende nieuwe Adobe Workfront- of Workflowpakketten:

   * Selecteren
   * Prime
   * Ultimate

  De Workfront Planning-test is niet beschikbaar voor de oudere Workfront-pakketten.
Voor informatie, zie [&#x200B; vereisten van de Toegang in de documentatie van Workfront &#x200B;](/help/quicksilver/administration-and-setup/add-users/access-levels-and-object-permissions/access-level-requirements-in-documentation.md).
* Accepteer de juridische evaluatieovereenkomst die beschikbaar is in het Workfront-exemplaar van uw organisatie tussen 26 januari en 1 april 2026. U moet een Workfront-beheerder zijn om de proefovereenkomst te accepteren.

## Overzicht van belangrijke datums

De volgende belangrijke datums zijn gekoppeld aan de gratis proefversie van Adobe Workfront Planning:

* &lbrace;26 januari 2026 **: De gratis proefbanners van Workfront Planning zijn vrijgegeven aan klanten van Workfront.** De banners omvatten het volgende:
   * Een koppeling naar dit document.
   * Het venster voor acceptatie van de proefovereenkomst. Alleen een Workfront-beheerder kan de overeenkomst accepteren. U kunt de proefovereenkomst op elk moment accepteren vanaf deze datum.
* **Maart 2, 2026**: De Workfront Planning-test is gestart.

  Met de start van de proefversie worden de volgende items toegevoegd aan uw Workfront-exemplaar:

   * De banners van de Planning van Workfront blijven voor alle gebruikers tonen. Een koppeling naar dit document wordt opgenomen in de banners.
   * De **proefovereenkomst van het Overzicht** voorkeur wordt toegevoegd aan het **3&rbrace; gebied van de Opstelling.**

  De volgende scenario&#39;s bestaan:

   * Als uw Workfront-beheerder de overeenkomst vóór deze datum heeft geaccepteerd, vindt u het planningsgebied in het hoofdmenu en begint u met het gebruik van Workfront Planning.

  >[!NOTE]
  >
  >Het gebied van de Planning toont in het Belangrijkste Menu voor alle gebruikers in het systeem, ongeacht hun de vergunningstype van Workfront.

   * Als uw Workfront-beheerder de overeenkomst vóór deze datum niet heeft geaccepteerd, worden de banners die de evaluatieprogramma&#39;s voor de planning bekendmaken voor alle gebruikers weergegeven, maar is Planning nog niet beschikbaar in het hoofdmenu. Uw systeembeheerder moet de overeenkomst eerst goedkeuren alvorens u toegang tot de Planning van Workfront kunt hebben.

* **1 April, 2026**: U kunt zich niet meer inschrijven voor de proefversie.

  De volgende items worden uit je Workfront-exemplaar verwijderd:

   * De testbanners van Workfront Planning.
   * De **proefovereenkomst van het Overzicht** voorkeur wordt verwijderd uit het **3&rbrace; gebied van de Opstelling.**

* **Mei 1, 2026**: De proefversie van de Planning van Workfront sluit en uw toegang tot Planning wordt verwijderd. Toegang blijft actief tot 15 mei 2026.

  Als u aan de proefversie hebt deelgenomen, beveiligt Workfront uw gegevens nadat de proefversie is afgelopen. U hebt er echter geen toegang meer tot na deze datum.

  Ongeacht wanneer u de overeenkomst goedkeurt, beëindigt uw toegang tot Planning op deze datum.

* &lbrace;30 november 2026 **: Workfront beveiligt uw gegevens na deze datum niet meer.** Als u vóór deze datum Workfront Planning aanschaft, krijgt u weer toegang tot Planning en uw gegevens.

<!--
Lauren wanted this out: 
* **November 30, 2026** - Workfront no longer makes your data available after this date. You can still purchase Workfront Planning, but your data is removed after this date.
-->

## Machtigingen voor planningsgegevens van Workfront tijdens en na de gratis proefperiode

Alle gebruikers in uw organisatie ontvangen het volgende Workfront-planningspakket en het niveau van de machtigingen voor Workfront Planning tijdens de proefperiode:

* **tussen 2 Maart - 1 mei, 2026**:

  Ongeacht het toegangsniveau van uw Workfront- of workflowlicentie, ontvangt u een Workfront Planning Prime-licentie als u wilt deelnemen aan het proefprogramma voor Workfront Planning.

  De gebruikers in uw systeem ontvangen de volgende toestemmingen aan de werkruimten in het gebied van de Planning tijdens de vrije proef:

   * Alle systeembeheerders hebben beheermachtigingen voor de werkruimten die ik aanwijs en het tabblad Alle werkruimten.
   * Alle andere gebruikers hebben de toestemmingen van de Mening aan het gebied van Werkruimten, maar de Beheerder van het Systeem kan hen verlenen leiden toestemmingen aan werkruimten die daar tonen.
   * Alle gebruikers, met inbegrip van de Beheerder van het Systeem, hebben de toestemmingen van de Mening aan het lusje van de werkruimten van de Steekproef in het Gebied van de Planning.

* **na 1 Mei, 2026:**

  Toegang tot de proef blijft actief tot 15 mei 2026. Na 15 mei zijn alle gebruikers in uw systeem verwijderd van Workfront Planning en beveiligt Workfront uw gegevens.

## Aanvullende informatie over Workfront Planning

Voor algemene informatie over de Planning van Workfront, zie [&#x200B; begonnen worden met de Planning van Adobe Workfront &#x200B;](/help/quicksilver/planning/general/planning-overview.md).

Voor beste praktijken op hoe te om de Planning van Workfront uit te voeren, zie [&#x200B; de Planning van Adobe Workfront beste praktijken: artikelindex &#x200B;](/help/quicksilver/planning/best-practices.md/best-practices-article-index.md) .

<!--

this information will be live on March 2 - the How to sign up below will be a ### instead of a ## section and the Navigate the trial section will be visible; also adjust ALL the ##s when you make this live:
-->

## Inschrijven voor de gratis proefversie van Adobe Workfront Planning

Een van uw Workfront-beheerders moet de juridische proefovereenkomst controleren en ondertekenen voordat iedereen in uw organisatie toegang krijgt tot de gratis proefversie van Workfront Planning.

Aanmelden voor de planningproef is sinds 26 januari 2026 beschikbaar.

De proef begon en uw organisatie ontving toegang om de proef van de Planning op 2 Maart, 2026 te beginnen.

>[!NOTE]
>
>Slechts één systeembeheerder moet de overeenkomst goedkeuren. Het is niet noodzakelijk voor alle Beheerders van het Systeem om het goed te keuren.

### Schrijf u tussen 26 januari en 2 maart 2026 in voor de planningproef

>[!WARNING]
>
>Deze periode is nu gesloten. Voor meer informatie, zie de sectie [&#x200B; Inschrijving in de proef van de Planning tussen 2 Maart en 1 April, 2026 &#x200B;](#enroll-in-the-planning-trial-between-march-2-and-april-1-2026) in dit artikel.

1. (Voorwaardelijk) Meld u aan bij Adobe Workfront als systeembeheerder.
1. Voer een van de volgende handelingen uit:

   * Volg in-app banner die informatie over de **Proefversie van de Planning van Workfront** bevat

   * Ga naar het **gebied van de Meldingen van Workfront** in de hoger-juiste hoek van het scherm, dan klik **Alle Mededelingen** en vind het bericht van het Centrum van de Aankondiging over de Proces van de Planning van Workfront.
1. Klik **de proefovereenkomst van het Overzicht**. <!--not sure if this will be available in the email/ banner, or if they can go to System Preferences to do this - might need to adjust the steps here-->
1. Klik **goedkeuren**, na het herzien van de overeenkomst.
1. (Voorwaardelijk) als u geen Beheerder van het Systeem bent, volg de in-app banner die informatie over de **Proefversie van de Planning van Workfront** bevat, dan klik **meer** leren.

   In dit artikel vindt u meer informatie over het Workfront Planning-proces en over Workfront Planning.

### Inschrijven in het planningproces tussen 2 maart en 1 april 2026

>[!WARNING]
>
>De opt-in-periode loopt af op 1 april 2026. De proeffuncties voor planning zijn echter beschikbaar tot 15 mei 2026.

1. (Voorwaardelijk) Meld u aan bij Adobe Workfront als systeembeheerder.

1. Voer een van de volgende handelingen uit:

   * Volg in-app banner die informatie over de **Proefversie van de Planning van Workfront** bevat

   * Ga naar het **gebied van de Meldingen van Workfront** in de hoger-juiste hoek van het scherm, dan klik **Alle Mededelingen** en vind het bericht van het Centrum van de Aankondiging over de proef van de Planning van Workfront

   * Ga naar **Opstelling** > **Systeem** > **Voorkeur**.
1. (Voorwaardelijk) als u op het **gebied van de Opstelling** bent, ga naar de **Andere voorkeur** sectie, dan klik **de proefovereenkomst van het Overzicht**.
1. Klik **goedkeuren**, na het herzien van de overeenkomst.

   Uw organisatie is nu ingeschreven voor de planningproef van Workfront tot 1 mei 2026. Toegang blijft actief tot 15 mei 2026.

   De volgende dingen treden op nadat u de proefovereenkomst hebt geaccepteerd:

   * Het planningsgebied wordt toegevoegd aan het Belangrijkste Menu voor u en alle gebruikers in uw systeem en het toont a **Proefindicator**.
   * Als Beheerder van het Systeem, ontvangt u Manage toegang tot het **lusje van de werkruimten van de Steekproef** in het planningsgebied.
   * De standaardgebruikers ontvangen de toegang van de Mening tot het **lusje van de Steekproef** in Planning en kunnen hun eigen werkruimten, verslagtypes, verslagen, gebieden en meningen tot stand brengen en hen met anderen delen.
   * Alle andere gebruikers ontvangen de toegang van de Mening tot het **lusje van de Steekproef** in het planningsgebied en kunnen toegang tot andere werkruimten krijgen wanneer anderen hen met hen delen.
1. (Voorwaardelijk) als u geen Beheerder van het Systeem bent, en uw Beheerder van het Systeem heeft de proefovereenkomst nog niet goedgekeurd, volg de in-app banner die informatie over de **Proefversie van de Planning van Workfront** bevat, dan klik **meer** leren.

   In dit artikel vindt u meer informatie over het Workfront Planning-proces en over Workfront Planning.
1. (Voorwaardelijk) als u geen Beheerder van het Systeem bent, en uw Beheerder van het Systeem heeft de proefovereenkomst goedgekeurd, volg de in-app banner die informatie over de **Proefversie van de Planning van Workfront** bevat en **Open Planning** klikt.

   Begin het onderzoeken van de **werkruimten van de Steekproef** tabel en overzicht, gebruik, of deelwerkruimten die met u worden gedeeld.

Voor meer informatie over het gebruiken van de Planning van Workfront, zie [&#x200B; begonnen worden met de Planning van Adobe Workfront &#x200B;](/help/quicksilver/planning/general/planning-overview.md).

Voor beste praktijken op hoe te om de Planning van Workfront uit te voeren, zie [&#x200B; de Planning van Adobe Workfront beste praktijken: artikelindex &#x200B;](/help/quicksilver/planning/best-practices.md/best-practices-article-index.md) .

## Feedback verzenden over planning

Feedback verzenden over je ervaring met Workfront Planning:

1. Meld u aan bij Workfront en open elke pagina.
1. Vind het korte onderzoek in de laag-juiste hoek van om het even welke pagina van Workfront en beantwoord de vragen, dan klik daarna **&#x200B;**.

   ![&#x200B; Promo proefonderzoek &#x200B;](assets/promo-trial-survey.png)

1. Voor de tweede dia, beantwoord de vragen, dan klik **voorleggen**.

   Uw feedback wordt verzonden naar ons productmanagementteam.


## Ervaar Workfront Planning via de gratis proefperiode

U kunt de werkruimten van de steekproefplanning en hun voorwerpen herzien, evenals uw tot stand brengen wanneer u in de Proces van de Planning van Workfront inschrijft.

1. (Voorwaardelijk en vereist) Als Workfront-beheerder ondertekent u de gratis proefovereenkomst.

   Voor informatie, zie de sectie [&#x200B; Inschrijving in de vrije proef van de Planning van Adobe Workfront &#x200B;](#enroll-in-the-adobe-workfront-planning-free-trial) in dit artikel.
1. (Voorwaardelijk) Nadat de proefovereenkomst door een beheerder van Workfront werd ondertekend, klik het **pictogram van het 1&rbrace; Belangrijkste Menu ![](assets/main-menu-shell.png), dan klik één van het volgende om tot het** Planning **gebied toegang te hebben:**

   * **Planning**. Het pictogram toont a **etiket van de a** Proefversie naast het.
   * **Projecten**, **Verzoeken**, **Kalenders**, dan klik **campagnecalenders onderzoeken**
   * **Portfolio&#39;s**, **Programma&#39;s**, of van een portefeuille of een programma dan klik **ontdekken douanehiërarchieën**.

   Het **Planning** gebied opent in de **werkruimten van de Steekproef** tabel.
1. Herzie de volgende werkruimten in de **werkruimten van de Steekproef** tabel:

   * **Globale Classificaties &amp; Taxonomies**: Bevat de objecten van de steekproefplanning objecttypes die de stichting van een Systeem van de Marketing van Verslag zijn.

     Alle recordtypen in deze werkruimte kunnen de bouwstenen van uw planningsstructuur van Workfront vormen. Alle recordtypen zijn globaal en kunnen worden toegevoegd of verbonden vanuit alle andere werkruimten. Voor meer informatie, zie [&#x200B; overzicht van het type van het werkruimterecord over de werkruimte &#x200B;](/help/quicksilver/planning/architecture/cross-workspace-record-types-overview.md).

     Voor onze aanbeveling over hoe te om de Globale werkruimte van Belastingen te gebruiken, zie [&#x200B; Draai uw eerste win in duurzame dynamiek: een playbook voor beheerde het schrapen &#x200B;](/help/quicksilver/planning/best-practices.md/playbook-how-to-scale.md).
   * Aanvullende voorbeeldwerkruimten: De volgende werkruimten dienen als voorbeelden van wat een steekproefbedrijf (Fréscopa) als specifieke werkruimten, verslagtypes, gebieden, en meningen aan architectuur zou kunnen nodig hebben hun organisatie en werkstructuur:

      * **Fréscopa Globale Marketing**
      * **Fréscopa Sociale Marketing**
      * **Fréscopa Media &amp; PR**
      * **Fréscopa Uitvoerend Bedrijfsleiding**

   >[!NOTE]
   >
   >Als Beheerder van het Systeem, zou u de steekproefwerkruimten kunnen uitgeven. Nochtans, adviseren wij u hen houden aangezien zij hen voor begeleiding moeten gebruiken en in plaats daarvan uw eigen werkruimten bouwen die die wij als steekproeven weerspiegelen.

1. Klik **creeer werkruimte** om uw eigen werkruimte tot stand te brengen.

   Voor informatie, zie [&#x200B; werkruimten &#x200B;](/help/quicksilver/planning/architecture/create-workspaces.md) creëren.

   Als Beheerder van het Systeem, uw nieuwe werkruimtevertoning op **Alle werkruimten** en **Werkruimten I op** lusjes is.

   >[!TIP]
   >
   >De standaard vergunningsgebruikers kunnen werkruimten tot stand brengen die in het **1&rbrace; gebied van de Werkruimten &lbrace;zullen tonen.**

1. Klik **creëren met AI** om de AI Medewerker te laten een werkruimte voor u bouwen, die op uw specificaties wordt gebaseerd. <!--(**********have they changed the button to Generate or is it Create???*********)-->

   Voor informatie, zie [&#x200B; begonnen worden met de Planning Designer van Adobe Workfront &#x200B;](/help/quicksilver/planning/general/planning-ai-designer.md).

   >[!IMPORTANT]
   >
   >We vereisen niet dat u de Adobe Gen AI-overeenkomst ondertekent om de Planning Designer in Workfront Planning tijdens het proces te kunnen gebruiken.

1. Maak een van de volgende werkruimten in de werkruimten die u hebt gemaakt:

   * Recordtypen

     Voor informatie, zie [&#x200B; recordtypes &#x200B;](/help/quicksilver/planning/architecture/create-record-types.md) creëren.
   * Records

     Voor informatie, zie [&#x200B; verslagen &#x200B;](/help/quicksilver/planning/records/create-records.md) creëren.
   * Weergaven

     Voor informatie, zie [&#x200B; verslagmeningen &#x200B;](/help/quicksilver/planning/views/manage-record-views.md) leiden.
   * Velden

     U kunt aangepaste velden maken voor elk recordtype, deze importeren uit Workfront of verbindingen maken met andere recordtypen, Workfront-objecttypen of objecttypen uit andere toepassingen.

     Raadpleeg de volgende artikelen voor meer informatie:

      * [Velden maken](/help/quicksilver/planning/fields/create-fields.md)
      * [Overzicht van Connect-recordtypen](/help/quicksilver/planning/architecture/connect-record-types-overview.md)

1. Deel in de werkruimten die u hebt gemaakt een van de volgende entiteiten:

   * Werkruimten

     Voor informatie, zie [&#x200B; de werkruimten van het Aandeel &#x200B;](/help/quicksilver/planning/access/share-workspaces.md).
   * Recordtypen

     Voor informatie, zie [&#x200B; recordtypes van het Aandeel &#x200B;](/help/quicksilver/planning/access/share-record-types.md).
   * Weergaven

     Voor informatie, zie [&#x200B; meningen van het Aandeel &#x200B;](/help/quicksilver/planning/access/share-views.md).

   Voor extra informatie over hoe te om de Planning van Workfront uit te voeren en hoe te om een centrum van bestuur voor het tot stand te brengen, zie de sectie [&#x200B; Aanvullende informatie over de Planning van Workfront &#x200B;](#additional-information-about-workfront-planning) in dit artikel.
1. (Optioneel) Voer een van de volgende handelingen uit om de gemaakte werkruimte te bewerken:

   * Open de werkruimte en breng de wijzigingen handmatig aan.

     Voor informatie, zie [&#x200B; werkruimten &#x200B;](/help/quicksilver/planning/architecture/edit-workspaces.md) uitgeven.
   * Klik **uitgeven met AI** naast de werkruimtenaam om **de Planning Designer** te openen en AI te gebruiken om verdere veranderingen in de werkruimte aan te brengen.

     Voor informatie, zie [&#x200B; begonnen worden met de Planning Designer van Adobe Workfront &#x200B;](/help/quicksilver/planning/general/planning-ai-designer.md).
1. (Facultatief) om het planningsgebied van het **Belangrijkste Menu** voor gebruikers te verwijderen, pas de **Malplaatjes van de Lay-out** aan de gebruikers worden toegewezen en verwijder het uit het **Vastgestelde Belangrijkste gebied van het Menu** van het Malplaatje van de Lay-out.

   Voor informatie, zie [&#x200B; het Belangrijkste Menu aanpassen gebruikend een lay-outmalplaatje &#x200B;](/help/quicksilver/administration-and-setup/customize-workfront/use-layout-templates/customize-main-menu.md).

   >[!TIP]
   >
   >U kunt de planningspromotiebanners niet verwijderen uit uw exemplaar van Workfront met behulp van lay-outsjablonen.


